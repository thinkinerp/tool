...
import com.alibaba.fastjson.JSONObject;
 public class Kpi_member_sale_weekly_json  {
  public String to_Kpi_member_sale_weekly_json( Connection conn) throws SQLException {
   Statement st = conn.createStatement();
  ResultSet rs = st.executeQuery("select * from `yonghuibi_s`.`kpi_member_sale_weekly` as t1 left join ( select max(week_num) as maxdate from `yonghuibi_s`.`kpi_member_sale_weekly` ) d on 1=1 where 1=1 and t1.store_id is null and t1.cat1_id is null and t1.channelname is NOT null and d.maxdate = t1.week_num order by t1.week_num ,t1.channelname");
  List<Kpi_member_sale_weekly> records = new ArrayList<Kpi_member_sale_weekly>();
  Kpi_member_sale_weekly vo = null;
 while(rs.next()){
       vo =  new Kpi_member_sale_weekly();vo.setStore_id(rs.getString("store_id"));
...
}public class Kpi_member_sale_weekly{
