...
import com.alibaba.fastjson.JSONObject;
 public class Kpi_member_sale_daily_json  {
  public String to_Kpi_member_sale_daily_json( Connection conn) throws SQLException {
   Statement st = conn.createStatement();
  ResultSet rs = st.executeQuery("select * from `yonghuibi_s`.`kpi_member_sale_daily` as t1 left join ( select max(sales_date) as maxdate from `yonghuibi_s`.`kpi_member_sale_daily` ) d on 1=1 where 1=1 and t1.store_id is not null and t1.cat1_id is null and t1.channelname is null and d.maxdate = t1.sales_date order by t1.sales_date ,t1.store_name group by t1.sales_date ,t1.store_name");
  List<Kpi_member_sale_daily> records = new ArrayList<Kpi_member_sale_daily>();
  Kpi_member_sale_daily vo = null;
 while(rs.next()){
       vo =  new Kpi_member_sale_daily();vo.setStore_id(rs.getString("store_id"));
...
}public class Kpi_member_sale_daily{
